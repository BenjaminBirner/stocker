...
/**
 * This class paints all the points that should be depicted and draws the line in a suitable coordinate system.
...
public class LineChartPanel extends JPanel {  
	...
	public LineChartPanel(ChartDataModel chartDataModel, AlarmDataModel alarmModel, IndicatorModel indiModel, String symbol, String resolution) {
	...
	protected void paintComponent(Graphics g) {
		...
		double min = chartDataModel.getMinClose(symbol, resolution);
		double max = chartDataModel.getMaxClose(symbol, resolution);
		//checks if the min close or the max close is out of range regarding the scale. 
		...
		//the canvas-width is divided into ELEM_NUMBER-1 parts. stepW saves the size of such a part in pixel.
		double stepW =(double) canvasW / (Chart.ELEM_NUMBER - 1);
		...
		int resoX = (getWidth() - Chart.SPACE_WEST - Chart.SPACE_EAST) / 2 - 50;
		...
		/*
		 * The following section draws the line
		 */
		...
		BasicStroke stroke = new BasicStroke(strokeSize,BasicStroke.CAP_BUTT,BasicStroke.JOIN_MITER);
		g2.setStroke(stroke);
		g2.setColor(Color.BLACK);
		
		Path2D path = new GeneralPath();
		
		//gets the required close-prices from the model
		double[] close = chartDataModel.getAllClose(symbol, resolution);
		// y1 defines the number of pixel from the bottom of the Panel to the first point.
		double y1 = (close[0] - scale0) / stepH + Chart.SPACE_SOUTH;
		//sets the first point
		path.moveTo(Chart.SPACE_WEST, -y1);
		double x1 = 0;
		//this loop calculates ands sets all the remaining points
		for(int i = 1; i < Chart.ELEM_NUMBER; i++) {
			// y1 defines the number of pixel from the bottom of the Panel to the i-th point.
			y1 = (close[i] - scale0) / stepH + Chart.SPACE_SOUTH;
			//the x-coordinate for the i-th point 
			x1 = stepW * i;
			//sets the i-th point
			path.lineTo(Chart.SPACE_WEST + x1, -y1);
		}
		//draws the line
		g2.draw(path);
		...
		BasicStroke stroke3 = new BasicStroke(strokeSize);
		...
		for(int i = 0; i < indiObjs.length; i++) {
			...
			if(indiObjs[i] != null) {
				...
				for(int h = 0; h < indis.length; h++) {
					...
					//sets the first point
					indiPath.moveTo(Chart.SPACE_WEST, -indiY);
					double indiX = 0;
